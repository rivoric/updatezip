...
so ren file newname would be ['ren','file','newname']"""
    ...
    c = command[0].lower()
    if c in __uz_aliases and len(command) > __uz_params[c]:
        compiled.append(__uz_aliases[c])
        if c == 'R': # Regex command
            compiled.append(re.compile(command[1]))
            compiled.append(command[2])
            compiled.append(command[3])
            itemsused = 4
        elif c == 'A': # Add command
            compiled.append(command[1])
            itemsused = 2
        elif c == 'D': # Delete command
            compiled.append(re.compile(command[1]))
            itemsused = 2
        elif c == 'U': # Update command
            compiled.append(re.compile(command[1]))
            compiled.append(command[2])
            itemsused = 3
        elif c == 'M': # Rename (move) command
            compiled.append(re.compile(command[1]))
            compiled.append(command[2])
            itemsused = 3
...
outfile   new filename path"""
    for jarfilename in jarfiles:
        if os.path.exists(jarfilename):
                ...
                for command in commands:
                    ...
                    if command[1].match(jfile):
                        ...
                        if command[0] == 'D':
                            ...
                            print("Updating %s - to be implemented" % (jfile))
                if data:
            ...
            for command in commands:
                if command[0] == 'A':
                    print("Adding file %s - to be implemented" % (command[1]))
